...
| `abilities/helpers/chooser.js` (direct DOM call for Kangaroo choice) | **Refactored** | Replace direct `ui/kangaroo-ui.js` import with an injected `InputProvider` interface. Local play still shows the modal; this is what makes the engine reusable server-side later without pulling in DOM code. *(Partial start, 1.31.0: `chooseKangarooJump()` now also accepts a `{ preview: true }` option that returns the farthest legal jump without touching the DOM at all — added for the Ability Preview System, see README § Ability Preview System. This is a narrower, single-purpose escape hatch, not the general `InputProvider` interface described here — that abstraction is still future work.)* |
...
| `js/game/scoreManager.js` (was a dead, empty file) | **Implemented (client-side half only)** | Now the real, single implementation of party-count/power ranking (`getPartyScore`/`getRankedPlayers`/`getPlayerRankIndexes`), consumed by `leaderboard-ui.js`'s Match Standings panel and by the opponent rank badges added to `game-ui.js`'s `renderOtherPlayers()` (1.32.0) — fixes the audit's duplication finding for those two call sites. `endgame-ui.js`'s final-results screen keeps its own top-3-only medal display unconsolidated (a deliberately different presentation, not more duplicated math worth merging). The server-side equivalent described here (for a future authoritative reward calc) is still not built — this phase only addresses the client-side duplication. |
