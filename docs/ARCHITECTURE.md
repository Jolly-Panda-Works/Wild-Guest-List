...
2. ~~**`data/cardInfo.json`'s Monkey description** ("...removes all Crocodiles and Hippos and moves them to the front") does not match the implementation, which sends them to Trash~~ — **fixed** in the tutorial/help content audit (see `CHANGELOG.md`); `docs/ANIMAL_ABILITIES.md`'s Monkey entry now records the correction.
...
6. **`js/ui/cardHelpHint.js` and `js/ui/longPress.js` are orphaned.** `cardHelpHint.js` exports `maybeShowCardHelpHint()` (meant to teach players, once, that holding a card opens Card Info) and `js/ui/longPress.js` exports the generic `attachLongPress()` gesture helper it's built on — but neither is imported or called from anywhere in `js/ui/game-ui.js` or any boot path. No hand card currently has a long-press handler wired to it at all; a plain tap/hold on a hand card does nothing except (if held long enough to become a drag) start the drag-to-play gesture. Discovered during the tutorial/help content audit (`CHANGELOG.md`) while fixing `cardHelpHintText`'s missing i18n entry — the string is now correct, but the feature it belongs to still never fires for any player. Needs an implementation decision: either wire `attachLongPress()` onto hand cards in `game-ui.js` to open the existing Card Info modal (`js/game/help.js`'s `openCardInfo`), or remove the dead `cardHelpHint.js`/unused parts of `longPress.js` if the feature is no longer wanted. Not fixed here — wiring a new gesture into `game-ui.js` is a UI-behavior change beyond this task's tutorial/help-content scope.
