// ══════════════════════════════════════════════════════════
// Game Modes page bootstrap — js/game-modes-main.js
//
// Entry point for game-modes.html — a top-level destination reached
// from Home's Offline/Online Game buttons (js/ui/home-ui.js), not
// rendered inside Home. "Play vs Bot" is the only implemented mode;
// choosing it reveals the same bot-difficulty panel Home used to
// host directly, right here as a sub-state of THIS page (a
// #play-vs-bot history entry, not a further top-level destination —
// see revealDifficultyPanel()/collapseDifficultyPanel() below).
...
// changed, only where the picking-a-mode UI lives.
// ══════════════════════════════════════════════════════════
...
import { showWarning } from "./ui/game-ui.js";
...
// content/logic to before, just living on its own page now.
...
// A sub-state of Game Modes, not a further top-level destination —
// but it still gets its own history entry (a #play-vs-bot hash) so
// Back correctly collapses to the mode tiles first, then returns to
// Home on a second Back, matching every other destination's history
// behavior instead of skipping a step.
function showTiles() {
    document.getElementById("modeTiles")?.classList.remove("hidden");
    document.getElementById("difficultyPanelSection")?.classList.add("hidden");
}

function showDifficultyPanel() {
    document.getElementById("modeTiles")?.classList.add("hidden");
...
// Deep link / refresh support: opening game-modes.html#play-vs-bot
// directly (e.g. Home's Offline Game button) lands straight on the
// difficulty panel instead of requiring the tiles to be shown first.
if (location.hash === PLAY_VS_BOT_HASH) {
    showDifficultyPanel();
}

document.getElementById("modePlayVsBotBtn")?.addEventListener("click", revealDifficultyPanel);
...
// ── Hand off to Game ──────────────────────────────────────
// The only thing that needs to cross the Game Modes → Game
// navigation is which difficulty each bot was set to. Everything
// else Game needs (player identity, sound/guidance settings,
// language) it reads itself from the same persisted stores used here.
